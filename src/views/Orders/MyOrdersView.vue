  ...
  methods: {
   loadOrders(ordering=null){
    if(ordering){
      ...
      this.pageUrl = '/api/orders/' + ordering
    }
    ...
    loadNextPage() {
      ...
      this.pageUrl = "/api/orders/?page=" + nextPage;
      if(this.ordering){
        this.pageUrl = "/api/orders/" + this.ordering + "&page=" + nextPage;
      }
      this.loadOrders();
    },
    loadPreviousPage() {
      const previousPage = parseInt(this.paginationInfo.currentPage) - 1;
      this.pageUrl = "/api/orders/?page=" + previousPage;
      if(this.ordering){
        this.pageUrl = "/api/orders/" + this.ordering + "&page=" + previousPage;
      }
      this.loadOrders();
    },

    loadRequestedPage(page) {
      this.pageUrl = "/api/orders/?page=" + page;
      if(this.ordering){
        this.pageUrl = "/api/orders/" + this.ordering + "&page=" + page;
      }
      this.loadOrders();
